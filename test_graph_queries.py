...
class TestReturnValueParsing(unittest.TestCase):
    ...
    def test_shortest_route_not_found_shape(self):
        ...
        self.assertIsNone(result["total_time_min"])
        self.assertEqual(result["path"], [])
        self.assertEqual(result["legs"], [])
        self.assertEqual(result["origin_id"], "MS01")
        self.assertEqual(result["destination_id"], "MS99")
    ...
    def test_shortest_route_found_shape(self):
        ...
        self.assertEqual(result["total_time_min"], 8)
        self.assertEqual(result["origin_id"], "MS01")
        self.assertEqual(result["destination_id"], "MS02")
        self.assertEqual(result["path"][0]["station_id"], "MS01")
        self.assertEqual(len(result["path"]), 2)
        self.assertEqual(len(result["legs"]), 1)
        self.assertEqual(result["legs"][0]["from_station_id"], "MS01")
        self.assertEqual(result["legs"][0]["to_station_id"], "MS02")
    ...
    def test_cheapest_route_not_found_shape(self):
        ...
        self.assertIsNone(result["total_fare_usd"])
    ...
    def test_interchange_path_found_shape(self):
        ...
        self.assertTrue(result["found"])
        self.assertEqual(result["total_time_min"], 10)
    ...
    def test_delay_ripple_returns_list_of_dicts(self):
        rows = [
            {"station_id": "MS02", "name": "North Gate", "label": "MetroStation", "hops_away": 1, "lines_affected": ["M1"]},
            {"station_id": "MS03", "name": "East End",   "label": "MetroStation", "hops_away": 2, "lines_affected": ["M2"]},
        ]
        ...
        self.assertEqual(result[0]["hops_away"], 1)
        self.assertEqual(result[0]["lines_affected"], ["M1"])
